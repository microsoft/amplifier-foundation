...
class MockCalendar:
    ...
    def find_free_slots(self, date: datetime, duration_minutes: int = 60) -> list[dict[str, Any]]:
        ...
        for hour in business_hours:
            ...
            conflicts = any(
                not (slot_end <= e["start"] or slot_start >= e["end"])
                for e in day_events
            )
...
Return as JSON with keys: title, attendees, duration, preferred_time, requirements"""
    ...
    async def find_best_time(self, meeting_data: dict[str, Any]) -> dict[str, Any]:
        ...
        slots_desc = "\n".join([
            f"{i+1}. {slot['start'].strftime('%A, %B %d at %I:%M %p')} - "
...
Respond with just the slot number (1-{len(all_slots)})."""
    ...
    async def create_meeting_invitation(self, meeting_data: dict[str, Any], 
...
async def scenario_check_availability():
    ...
    for event in events:
        print(f"  {event['start'].strftime('%I:%M %p')} - "
              f"{event['end'].strftime('%I:%M %p')}: {event['summary']}")
    ...
    for i, slot in enumerate(free_slots, 1):
        print(f"  {i}. {slot['start'].strftime('%I:%M %p')} - "
              f"{slot['end'].strftime('%I:%M %p')}")
    
    # Ask AI for recommendation
    slots_desc = "\n".join([
        f"{i}. {slot['start'].strftime('%I:%M %p')}"
        for i, slot in enumerate(free_slots, 1)
    ])
