...
class PreparedBundle:
    ...
    async def create_session(
        ...
        #
        # Important limitation: the public API does NOT expose handler callables or priorities.
        # As a result, SessionConfigurator.hook_disable() works (uses public unregister(name)),
        # but hook_enable() cannot re-register a hook without a handler reference. Attempting
        # hook_enable() on a partially-captured hook raises RuntimeError with a clear message.
        #
        # The right long-term fix is to add list_handlers() metadata to include callable refs,
        # or expose a separate introspection API in amplifier-core. Until then, this is the
        # least-bad approach that uses only the stable public API and fails explicitly.
        try:
            hook_data = session.coordinator.hooks.list_handlers()
            hook_metadata: dict[str, dict[str, Any]] = {
                name: {"event": event}
                for event, names in hook_data.items()
                for name in names
            }
            session.coordinator.register_capability("hook_metadata", hook_metadata)
            logger.debug(
                "Captured %d hook registration(s) for SessionConfigurator",
                len(hook_metadata),
            )
        except Exception as exc:
            logger.debug(
                "Could not capture hook metadata for SessionConfigurator: %s", exc
            )
