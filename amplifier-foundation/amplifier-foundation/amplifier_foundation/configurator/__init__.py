...
class SessionConfigurator:
    ...
    def hook_enable(self, name: str) -> None:
        ...
        if handler is None:
            raise RuntimeError(
                f"Cannot re-enable hook {name!r}: handler reference not captured. "
                f"The public list_handlers() API does not expose callables. "
                f"Restart the session to restore this hook."
            )
