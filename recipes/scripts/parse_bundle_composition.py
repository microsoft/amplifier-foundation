...
def discover_component_files(local_path: Path) -> list[tuple[Path, str]]:
    ...
    if local_path.is_file():
        ...
        if _looks_like_bundle_dir(parent):
            parent_entries = discover_component_files(parent)
...
def resolve_behavior_includes(
    ...
    def _resolve_ns_path(ref: str, for_agents: bool = False) -> Path | None:
        """Resolve a 'namespace:relative_path' reference to an absolute Path.
    ...
    for ref in behavior_fields.nested_behavior_includes:
        try:
            ...
            logging.warning(
                "resolve_behavior_includes: failed to resolve nested behavior"
                " '%s': %s",
                ref,
                exc,
            )
                ...
                if registry_path is not None:
                    ...
                    if cache_dir.is_dir():
                        candidates: list[Path] = sorted(
                            ...
                            key=lambda p: (
                                p.stat().st_mtime if p.is_dir() else 0
                            ),
                            reverse=True,
                        )
                        for candidate in candidates:
                            if candidate.is_dir() and _looks_like_bundle_dir(
                                candidate
                            ):
                                logging.warning(
