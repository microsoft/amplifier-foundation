...
class TestIntegration:
    ...
    def test_real_foundation_manifest(self):
        ...
        # Should have entries for the modes bundle's modes (careful, explore, plan)
        assert len(result["tool_availability_matrix"]) >= 3
        # Check for modes from the modes bundle (not superpowers-specific)
        assert any(
            mode in result["tool_availability_matrix"]
            for mode in ("careful", "explore", "plan")
        )
        ...
        assert len(result["skill_mode_associations"]) >= 1
