...
# -- Task E: git+https:// URL ref resolution in resolve_behavior_includes -----


class TestGitHttpsUrlResolution:
    """resolve_behavior_includes resolves git+https:// URL refs."""

    def test_resolves_nested_behavior_via_git_url(self, tmp_path):
        """A nested_behavior_includes entry using a git+https:// URL with
        #subdirectory= is resolved to the local file via the registry URI."""
        # Set up the "modes" bundle on disk
        ...
        # Registry: the modes bundle is registered by name with a URI and local_path
        bundles = {
            "my-bundle": {
                "name": "my-bundle",
        ...
        # The behavior YAML that includes the modes bundle via git+https:// URL
    ...
    def test_resolves_agent_include_via_git_url(self, tmp_path):
        """An agent_includes entry using a git+https:// URL is resolved."""
        # Set up the bundle on disk with an agent
    ...
    def test_resolves_context_include_via_git_url(self, tmp_path):
        """A context_includes entry using a git+https:// URL is resolved."""
    ...
    def test_git_url_no_matching_bundle_returns_empty(self):
        """A git+https:// URL that matches no bundle URI is silently skipped."""
        bundles = {
            "some-bundle": {
                "name": "some-bundle",
    ...
    def test_git_url_without_subdirectory_resolves_to_bundle_root(self, tmp_path):
        """A git+https:// URL without #subdirectory= resolves to the bundle root."""
        bundle_dir = tmp_path / "root-bundle"
        bundle_dir.mkdir()
        # Make it look like a behavior YAML at the root level
        # (an edge case -- if no subdirectory, we get the base dir itself)
        behaviors_dir = bundle_dir / "behaviors"
        behaviors_dir.mkdir()
        # Since the resolved path is the directory itself, it won't be parsable
        # as a single behavior YAML -- this tests that the resolution succeeds
        # without error but the parse may fail (and be silently skipped).
        bundles = {
            "root-bundle": {
                "name": "root-bundle",
        ...
        # Should not raise -- directory resolve returns the dir, parse will fail
        # gracefully and produce no results
        results = resolve_behavior_includes(behavior_fields, bundles, "my-bundle")
        # The result is either 0 (parse fails on directory) or could succeed
        # depending on the file, but should never raise
