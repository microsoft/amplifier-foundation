...
from amplifier_foundation.bundle_docs.bundle_to_dot import (
    _get_repo_git_url,
...
class TestPackageImports:
    """Verify the v3 bundle_docs package __init__.py exports."""

    def test_import_bundle_repo_dot(self) -> None:
        from amplifier_foundation.bundle_docs import bundle_repo_dot as _fn  # noqa: F401

        assert callable(_fn)

    def test_import_estimate_tokens(self) -> None:
        from amplifier_foundation.bundle_docs import estimate_tokens as _fn  # noqa: F401

        assert callable(_fn)

    def test_import_parse_frontmatter(self) -> None:
        from amplifier_foundation.bundle_docs import parse_frontmatter as _fn  # noqa: F401
...
class TestAllExportsCallable:
    ...
    def test_all_exports_callable(self) -> None:
        import amplifier_foundation.bundle_docs as bundle_docs

        assert hasattr(bundle_docs, "__all__"), "bundle_docs must define __all__"
        for name in bundle_docs.__all__:
            obj = getattr(bundle_docs, name, None)
            assert obj is not None, f"bundle_docs.{name} missing after import"
            assert callable(obj), f"bundle_docs.{name} is not callable"
